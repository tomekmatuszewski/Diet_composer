...
class TestRecipeView:
    ...
    def create_recipe(self, django_db_blocker, django_db_setup, user):
        with django_db_blocker.unblock():
            ...
            recipe = Recipe.objects.create(
                ...
                preparation_time="10min", ingredients="test ingredients",
                category=category)
        yield recipe
        with django_db_blocker.unblock():
            recipe.delete()
    ...
    def test_recipe_create_view(self, client, recipe):
        ...
        response = client.post(reverse('recipe-create'),
                               data={
    ...
    def test_recipe_update_view(self, client, recipe):
        ...
        client.login(username="test_user", password="test12345")
        response = client.post(reverse('recipe-update', kwargs={"pk": recipe.pk}),
                               data={
                                   "category": recipe.category.id,
...
class TestLikeView:
    ...
    def test_like_view(self, recipe, client):
        client.login(username="test_user", password="test12345")
        response = client.post(reverse("recipe-like", kwargs={"pk": recipe.pk}), {"recipe_id": recipe.pk})
        assert response.status_code == 302
